...
const {
  ...
  maskMysqlConfig
} = require('../../../../core/data/db');
const {
  setMasterKey,
  clearMasterKey,
  getMasterKeySource
} = require('../../../../core/data/db-secrets');
...
const REQUIRED_FILES = [
  ...
  'Features/modules/admin/web/update-download.html'
];
...
const buildPackagePageTargets = (normalizedEntries) => {
  ...
  applyPathByLower.forEach((relativePath) => {
    const publicMatch = /^public\/([^/]+)\.html$/i.exec(relativePath);
    const moduleMatch = /^Features\/modules\/([^/]+)\/web\/([^/]+)\.html$/i.exec(relativePath);
    if (!publicMatch && !moduleMatch) return;

    const moduleId = normalizePageKey(moduleMatch?.[1]);
    const assetKey = normalizePageKey(publicMatch?.[1] || moduleMatch?.[2]);
    const key = moduleId ? `${moduleId}-${assetKey}` : assetKey;
    if (!key) return;

    const basePath = moduleId
      ? `Features/modules/${moduleMatch[1]}/web`
      : 'public';
    ...
    if (!pageMap.has(key)) {
      pageMap.set(key, {
        ...
        basePath,
        label: moduleId
          ? `${titleFromPageKey(moduleId)} · ${titleFromPageKey(assetKey)}`
          : titleFromPageKey(assetKey),
  ...
  pageMap.forEach((page) => {
    const key = page.assetKey;
    const basePath = page.basePath;
    [
      `${basePath}/${key}.js`,
      `${basePath}/${key}.css`,
      `${basePath}/js/${key}.js`,
      `${basePath}/css/${key}.css`
