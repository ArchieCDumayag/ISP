(() => {
  ...
  const setResult = (message = '', type = 'info') => {
    ...
    refs.result.setAttribute('role', type === 'danger' ? 'alert' : 'status');
    refs.result.setAttribute('aria-live', type === 'danger' ? 'assertive' : 'polite');
    if (message && type === 'danger') refs.result.focus();
  };

  const setLoading = (loading) => {
    refs.form.setAttribute('aria-busy', loading ? 'true' : 'false');
    [
      refs.apk,
      refs.sourceUrl,
      refs.versionName,
  ...
  const loadStatus = async ({ announce = false } = {}) => {
    ...
    try {
      ...
      renderStatus(data);
      const message = currentUpdate
        ? `Collector update status refreshed. Current release: ${currentUpdate.versionName}, version code ${currentUpdate.versionCode}.`
        : 'Collector update status refreshed. No release is currently published.';
      setResult(announce ? message : '', 'success');
      loaded = true;
    } catch (error) {
      setResult(error.message || 'Failed to load Collector update status.', 'danger');
    } finally {
      refs.refresh.disabled = false;
      if (loaded && announce) refs.refresh.focus();
    }
  };
  ...
  refs.form.addEventListener('submit', async (event) => {
    ...
    const requiredPolicy = refs.required.checked ? 'Required update' : 'Optional update';
    const minimumCode = Number(refs.minimumCode.value || 0);
    const confirmation = `Publish THRE3J Collector ${refs.versionName.value.trim()} (version code ${versionCode}) to all phones? ${requiredPolicy}; minimum supported version code ${minimumCode}.`;
    ...
    try {
      ...
      setLoading(false);
      if (published) refs.publish.focus();
    }
  });

  refs.refresh.addEventListener('click', () => loadStatus({ announce: true }));
  loadStatus();
})();
