...
- For a stopped-billing disconnection, the full Payment Breakdown **Reconnect** action opens a Tabler settlement modal instead of directly clearing the disconnection. It previews the separate previous-balance treatment, reconnection proration or next-cycle start, next regular bill, and activation rule; it blocks same-month proration when a regular charge already exists and requires an audit reason and confirmation. A payment-gated request remains disabled and displays the remaining activation amount until collected payments reach the threshold.
...
- Reconnection settlements are stored as sanitized audit history on the branch/account disconnection decision. A stopped interval never creates catch-up cycles. The previous balance remains a separate ledger component and is either retained, written off once, or deferred into an exact cent-balanced schedule; reconnection proration is a separate calculated row, and the following prepaid first-day or postpaid month-end bill remains a separate regular cycle. Pending-payment settlements create no charge, count only new collected-payment entries, protect those activation payments from deletion, and rebuild proration plus the next billing/due dates from the actual activation day if payment arrives later.
