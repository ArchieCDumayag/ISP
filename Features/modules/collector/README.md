...
- Canonical backend implementations: `backend/collector-next-due.js`, `backend/collector-live-updates.js`, `backend/collector-payments.js`, `backend/collector-priorities.js`, `backend/collector-reschedules.js`, `backend/collectors.js`, and `backend/routes/collectors.js`
...
- Main APIs: `/api/collectors` (including the Admin live-update stream at `/events`), `/api/collector/payments` (including `/priorities` and `/reschedules`), plus collector session endpoints under `/api/auth`
...
- Collector schedule reads are authoritative active-only snapshots for authenticated Collector accounts, even when an older app requests `status=all`. Deleted, paid, replaced, and otherwise closed schedules remain available to Admin history but are sent to Android only as identifier-only removal tombstones.
