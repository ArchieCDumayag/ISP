...
- `backend/pon-serviceability.js` supplies both the distance-limited online/free-port candidate contract and an expanded-limit coverage-map mode that still enforces its requested radius while retaining full or offline NAPs inside it, with a numbered `ports` roster. Each port is classified as `available`, `occupied`, `reserved`, or `unavailable`; only an online unoccupied/unreserved port is selectable in Android. New-customer onboarding records that choice as an advisory request and creates no reservation or expiry. Admin approval acquires the branch lock, revalidates the reviewed NAP/port against canonical assignments, legacy active/draft-held reservations, and mapped Temp occupancy, then creates the canonical assignment atomically or returns a conflict requiring another port. Legacy reservation APIs remain for older installed-client/assigned-job compatibility. Customer labels remain internal until the Technician route applies its dedicated coverage-map sanitizer.
- PON revisions hash only persisted OLT, NAP, and port-assignment fields, excluding live/derived subscriber status. JSON comparisons run under the shared branch mutation lock. MySQL saves acquire the branch row and PON rows inside one transaction before comparing; legacy reservation mutations and new Admin requested-port finalization acquire that same branch lock. Existing temporary reservations and durable legacy draft holds continue protecting their topology, while a new onboarding request intentionally does not reduce availability until Admin approval succeeds.
...
- Temp supplies a sanitized branch-filtered mapped-customer provider for derived Admin PON/Coverage Map display and Technician serviceability occupancy checks. Network never writes Temp customer/billing data, and Temp assignments never enter canonical `pon_nap_connections`.
...
- 2026-08-30 Temp serviceability validation: focused pure and isolated JSON tests prove a mapped Temp assignment reduces the Android available-port count, appears as occupied with its sanitized label, blocks a new reservation, and blocks finalization when the Temp assignment appears after reservation.
- 2026-08-30 durable draft-hold validation: focused Network tests prove submission removes expiry, the held port continues blocking availability, technicians cannot release submitted holds, Admin reassignment is atomic, PON Management revisions protect holds, and Admin finalization/release remains replay-safe. Source compatibility checks cover the additive nullable-expiry MySQL migration and new hold APIs; the compatibility process exits after its final marker so imported runtime handles cannot stall Phase 12.
- 2026-08-30 requested-port validation: isolated JSON tests prove Admin can atomically assign a reviewed requested port without creating a reservation, exact replay is idempotent, and a second customer receives a conflict after the port is assigned.

## Latest meaningful changes

- 2026-08-30: Superseded reservation-based new-customer onboarding with a non-binding requested NAP/port. Android creates no hold; only Admin approval atomically assigns the reviewed port. Legacy reservation/hold contracts remain for compatibility.

- 2026-08-30: Extended the existing Temp network-customer provider into Technician PON serviceability. Android coverage now counts and labels mapped Temp ports as occupied, while atomic reserve/finalize checks reject those ports without copying Temp records into canonical PON state.

- 2026-08-30: Fixed the PON subscriber modal refresh so it retains merged read-only Temp assignments instead of replacing them with canonical-only `/state` data immediately after opening. Temp rows now carry a visible orange **TEMP** badge and remain non-editable; canonical serialization and PON persistence are unchanged.
