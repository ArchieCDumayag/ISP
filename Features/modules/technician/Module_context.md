...
- Rank nearby online NAPs with exact free ports, reserve one port temporarily, release it, and finalize the reserved assignment without replacing an occupied port, moving an existing subscriber, or rewriting the full PON state.
...
- `GET /api/technician/installations/pon/nearby?customerAccountNumber=...&latitude=...&longitude=...`: nearest online serviceable NAPs with exact available port numbers.
...
- Finalize may include a structured `installationCompletion` object with its own stable `clientEventId`, ONU serial, MAC, optical signal, cable length, material evidence, and notes. For technician-created pending clients, the server persists this evidence into the owned draft after the idempotent PON finalize. Persistence uses a JSON store-wide or MySQL row-locked compare-and-set: an exact replay returns the first record, while concurrent or later altered evidence is rejected without overwriting it.
...
- Installation completion evidence uses the Customer Draft store's atomic compare-and-set contract. JSON serializes the complete shared draft-file mutation, and MySQL locks the selected branch/account draft row before comparing the completion event and fingerprint.
...
- Android must retain one UUID-like `clientEventId` for every queued PON reservation/finalize or inventory mutation until the server acknowledges it. Material use sends a canonical `jobId`, never a label or free-form reference.
...
- PON reservations expire after the Network-owned TTL. Android must refresh nearby availability and create a new reservation after expiry; it must not attempt an override.
