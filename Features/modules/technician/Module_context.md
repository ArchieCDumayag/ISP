...
- Let a technician work only with their own in-progress or pending customer drafts or canonical customers on their assigned open jobs; this boundary applies to installation customer lists, PON serviceability/assignment, and PPPoE generation. New draft intake remains hidden from Admin until the technician submits the completed installation for review.
- Rank nearby online NAPs with exact free ports or load every NAP within the technician coverage radius, reserve one available port while completing the form, and submit it as a non-expiring Admin-review hold without replacing an occupied port, moving an existing subscriber, or rewriting the full PON state.
...
- `GET /api/technician/installations/pon/nearby?customerAccountNumber=...&latitude=...&longitude=...`: nearest online serviceable NAPs with exact available port numbers. Adding `coverageMap=true` returns every coordinate-valid branch NAP within a server-enforced 600-meter radius, including full/offline NAPs, and its complete port roster; new local onboarding may omit the account number because no server draft exists yet. Canonical and mapped read-only Temp assignments count as occupied. Occupied/reserved rows expose only sanitized labels, available rows expose no client identity, and legacy nearby remains client-redacted.
- `POST /api/technician/installations/pon/reservations`: reserve an exact NAP port for the authorized customer (`/pon/reserve` is a compatibility alias). Network rechecks canonical reservations/assignments and mapped Temp occupancy before creating the hold.
- `DELETE /api/technician/installations/pon/reservations/:reservationId` or `POST .../:reservationId/release`: release the authenticated technician's temporary customer-bound reservation (`/pon/release` is an alias). A submitted `draft-held` reservation is Admin-controlled and cannot be released by the technician.
- New Android onboarding sends customer, billing, GPS, requested NAP/port, ONU serial, and stable events together to `POST /api/customer-drafts`. The server creates internal intake and transitions it to Admin-visible Pending in that one request; no port reservation or expiry is created. Legacy PON submission/reservation/finalize aliases remain only for older installed-client and assigned-job compatibility.
- New `installationCompletion` submissions require a stable `clientEventId` and ONU serial. The server strips serial whitespace, uppercases it, records the selected NAP/port as `requested`, and retains older evidence only for legacy clients. Admin may edit the selection; only approval atomically validates and creates the canonical assignment while promoting the trusted ONU serial.
...
Customer Management supplies customer snapshots, one-request draft intake, canonical customer coordinates, and technician login/session records. Admin supplies account roles. Network supplies branch-scoped `/api/pon/state`, coverage candidates, legacy reservation operations, and Admin-only requested-port finalization. Billing retains installation dependencies.
...
- `/job-history.html` and `/technician-customer-drafts.html` retain their existing responsibilities and URLs. The customer draft page loads shared `/js/customer-name-case.js`, so first/last names auto-format on blur/submit before Customer Management applies the same authoritative backend rule.
...
- Installation completion history uses the Customer Draft store's atomic compare-and-set contract. The final Android POST creates internal `in-progress` intake and immediately applies immutable completion with `transitionToPending`, so Customer Draft Queue receives only the complete customer/billing/GPS/requested-port/ONU record. JSON serializes the shared mutation and MySQL locks the branch/account row. The canonical customer stores normalized `onuSerialNumber` only after Admin approval.
...
- Android retains one stable customer event and one stable completion event until the final draft POST is acknowledged. Opening the map and choosing a port only update encrypted local draft state; the final action posts every onboarding field to `/api/customer-drafts` once and identifies the NAP/port as an unreserved request that Admin may change. Legacy reservation routes remain compatible but the new UI does not call them.
...
- New onboarding has no reservation or expiration. Android displays current availability only; the request may be taken before review. Admin must select another available port when necessary, and approval revalidates/assigns under the Network branch lock. Legacy reservation TTL behavior applies only to compatibility clients.
...
- The 2026-08-30 requested-selection regression proves coverage-map mode works before an account exists, one complete draft transitions to Pending, Admin owns final port assignment, and exact retry remains stable. Legacy installed-client reservation contracts remain covered separately.
- The 2026-08-30 submission-error regression verifies an owned internal `in-progress` intake is authorized through the same Technician boundary as an owned `pending` draft. This prevents first submission from falling through to assigned-job lookup and returning a false customer-not-found response.
...
- 2026-08-30: Added shared customer-name auto-formatting to the technician draft page. Names are converted to safe Title Case before submit and normalized again by Customer Management; installation, account, NAP, and approval behavior is unchanged.

- 2026-08-30: Fixed first-time Android submission for the new hidden-intake lifecycle. Technician authorization now recognizes the submitting technician's `in-progress` row before it transitions to Pending, eliminating the false 404 that made the client discard a valid reservation.

- 2026-08-30: Changed Android installation completion from immediate PON finalization to a non-expiring Admin-review hold. The technician still selects a currently available NAP/port, but only a complete submission enters Customer Draft Queue; Admin may change the held NAP/port before approval and is the only actor that finalizes or releases it.

- 2026-08-30: Corrected Android NAP availability for mapped Temp subscribers. Coverage counts and labels their ports as occupied, and Network now rejects reserve/finalize if a Temp assignment owns the selected port, including assignments created after the technician loaded the map.
