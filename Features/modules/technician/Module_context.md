...
- Finalize may include a structured `installationCompletion` object with its own stable `clientEventId`, ONU serial and supported brand, optical signal, drop-cable start/end meter readings, server-computed cable length, an installed-material checklist/quantities, additional assigned-inventory evidence, and notes. The Android form no longer collects or sends a router/ONU MAC; the server still normalizes a legacy MAC when an older replay includes one. For technician-created pending clients, the server persists this evidence into the owned draft after the idempotent PON finalize. Persistence uses a JSON store-wide or MySQL row-locked compare-and-set: an exact replay returns the first record, while concurrent or later altered evidence is rejected without overwriting it.
