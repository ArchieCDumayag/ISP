...
- Temp customer, balance, import, and export operations never read or write the canonical `customers`, `payments`, or `plans` stores. Official GCash posting uses Billing's lightweight read-only lookup for collected or pending Main references—including a Cash/blank-method credit only when its reference, amount, and date exactly identify the official credit—and the shared imported-history claim/finalize contract so the same reference cannot become effective in both workspaces.
...
- Manual GCash creation is blocked; Admin uses the GCash Posting tab after importing the official PDF. A manual Cash/blank-method credit whose reference matches an incoming imported credit is also blocked so mislabeling cannot bypass the official claim. A reference already present in Main is warning-only and cannot be posted to Temp. One reference belongs to one workspace; mixed Main-and-Temp allocation is deliberately not performed by this isolated workflow.
