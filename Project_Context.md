...
- Android installation evidence is a Technician contract. New completion requests use an exact supported ONU-brand dropdown, omit router/ONU MAC, submit drop-cable start/end meter readings for server-side length computation, and record IOO, patch cord, connector, clip, tie, and clamp usage separately from optional assigned-inventory evidence. The server continues accepting legacy completion payloads without a brand and preserves a legacy MAC only for retry/history compatibility.
