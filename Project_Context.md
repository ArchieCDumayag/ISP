...
- The local-only `local-collector-ota` branch publishes permanent release-signed THRE3J Collector APKs at `/collector-app-update.html`. LAN phones read `http://192.168.100.9:3000/collector-updates/update.json` and the active versioned APK; generated files live under ignored `data/collector-updates` and are not source-controlled. The public deployment branch does not expose these routes.
...
- On the local-only branch, `/api/collector-app-updates` is Admin-only and publishes one validated Android APK with server-computed SHA-256, exact byte size, version metadata, optional required policy, and an activity audit. Admins can upload a local APK or import one from the repository-scoped `raw.githubusercontent.com/ArchieCDumayag/CollectorApp/` HTTPS source. LAN clients read the unauthenticated manifest/APK only from `http://192.168.100.9:3000/collector-updates`; the Android client restricts HTTP to that exact origin and verifies package ID, version code, archive signature match, and checksum before installation.
...
- 2026-09-01: Moved the Collector Android OTA publisher to the local-only `local-collector-ota` branch and pinned its manifest/APK origin to `http://192.168.100.9:3000/collector-updates`. The public production branch is reverted and no longer exposes the OTA page, API, manifest, or APK routes.
- 2026-08-31: Added an Admin-managed, server-hosted Collector Android OTA channel. The upload page reuses normal Admin authentication instead of separate webserver credentials, atomically publishes a version/checksum-bound manifest and APK under ignored runtime data, and exposes only the current release to unauthenticated phones for the app's independent package/signature/checksum verification.
