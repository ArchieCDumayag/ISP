...
- `/temp.html` and `/api/temp` are protected for Admin sessions and deliberately have no sidebar/dashboard link. Customer, ledger, backup, and report data remain physically isolated from canonical Customer Management and Billing. Official Temp GCash posting is the narrow exception: a lightweight Billing lookup blocks collected or pending Main GCash references and exact official credits mislabeled Cash/blank, then Temp claims/finalizes the shared branch imported-history reference before an atomic Temp write. Temp manual CRUD/import also rejects cross-method reuse of an incoming or already-owned GCash reference. Collector approval uses the same assignment authority, so a Temp-owned reference cannot later become effective there; rejection resolves canonical padded references before releasing an owned unposted claim. Temp never writes or exports a Main customer/payment row.
...
- One official GCash reference is assigned wholly to Main or Temp. The hidden Temp workflow supports one-to-three Temp allocations and blocks Main-reference conflicts, but deliberately does not attempt a mixed Main-and-Temp two-ledger transaction; that would require a separate coordinated saga and explicit scope.
