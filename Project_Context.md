...
- Admins publish permanent release-signed THRE3J Collector APKs at `/collector-app-update.html`. Public phones read `/collector-updates/update.json` and the active versioned APK; generated files live under ignored `data/collector-updates` and are not source-controlled.
...
- `/api/collector-app-updates` is Admin-only and publishes one validated Android APK with server-computed SHA-256, exact byte size, version metadata, optional required policy, and an activity audit. Admins can upload a local APK or import one from the repository-scoped `raw.githubusercontent.com/ArchieCDumayag/CollectorApp/` HTTPS source; the host and repository restriction prevents arbitrary server-side URL fetching. `/collector-updates/update.json` and its current checksum-named APK are public because phones must update before or without login; the Android client additionally verifies HTTPS, package ID, version code, archive signature match, and checksum before installation.
