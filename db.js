...
const { getStorageDriver, isMysqlStorageMode } = require('./storage-mode');

let mysql = null;
let pool = null;

const MYSQL_DATA_DIR = path.join(__dirname, 'data');
const MYSQL_CONFIG_FILE = path.join(MYSQL_DATA_DIR, 'mysql-config.json');
const MYSQL_CONFIG_BACKUP_FILE = path.join(MYSQL_DATA_DIR, 'mysql-config.backup.json');
